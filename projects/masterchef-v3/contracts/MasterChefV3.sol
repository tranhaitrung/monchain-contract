...
import "./interfaces/IMonPool.sol";
...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice Address of MOON contract.
    IERC20 public immutable MOON;

    /// @notice Address of WMON contract.
    ...
    uint256 public latestPeriodMoonPerSecond;
    ...
    /// @notice Record the moon amount belong to MasterChefV3.
    uint256 public moonAmountBelongToMC;
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, IMonPool indexed v3Pool, ILMPool indexed lmPool);
    ...
    /// @param _MOON The MOON token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(IERC20 _MOON, INonfungiblePositionManager _nonfungiblePositionManager, address _WMON) {
        MOON = _MOON;
        nonfungiblePositionManager = _nonfungiblePositionManager;
        WMON = _WMON;
    }

    /// @notice Returns the moon per second , period end time.
    /// @param _pid The pool pid.
    /// @return moonPerSecond Moon reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfoByPid(uint256 _pid) public view returns (uint256 moonPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            moonPerSecond = (latestPeriodMoonPerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice Returns the moon per second , period end time. This is for liquidity mining pool.
    /// @param _v3Pool Address of the V3 pool.
    /// @return moonPerSecond Moon reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfo(address _v3Pool) public view returns (uint256 moonPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            moonPerSecond =
                (latestPeriodMoonPerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) /
                totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice View function for checking pending MOON rewards.
    /// @dev The pending moon amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    /// @param _tokenId Token Id of NFT.
    /// @return reward Pending reward.
    function pendingMoon(uint256 _tokenId) external view returns (uint256 reward) {
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (MOON.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    function add(uint256 _allocPoint, IMonPool _v3Pool, bool _withUpdate) external onlyOwner {
    ...
    /// @notice Update the given pool's MOON allocation point. Can only be called by the owner.
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotMonswapNFT();
    ...
    /// @notice harvest moon from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return reward Moon reward.
    ...
    function increaseLiquidity(
        ...
        if (pool.token0 != WMON && pool.token1 != WMON && msg.value > 0) revert();
    ...
    function refund(address _token, uint256 _amount) internal {
        if (_token == WMON && msg.value > 0) {
            nonfungiblePositionManager.refundMON();
            safeTransferMON(msg.sender, address(this).balance);
        } else {
    ...
    /// @dev Warning!!! Please make sure to use multicall to call unwrapWMON9 or sweepToken when set recipient address(0), or you will lose your funds.
    ...
    function transferToken(address _token, address _to) internal {
        ...
        // Need to reduce moonAmountBelongToMC.
        if (_token == address(MOON)) {
            unchecked {
                // In fact balance should always be greater than or equal to moonAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balance >= moonAmountBelongToMC) {
                    balance -= moonAmountBelongToMC;
                } else {
                    // This should never happend.
                    moonAmountBelongToMC = balance;
                    balance = 0;
                }
            }
        }
        if (balance > 0) {
            if (_token == WMON) {
                IWMON(WMON).withdraw(balance);
                safeTransferMON(_to, balance);
            } else {
    ...
    /// @notice Unwraps the contract's WMON9 balance and sends it to recipient as MON.
    /// @dev The amountMinimum parameter prevents malicious contracts from stealing WMON9 from users.
    /// @param amountMinimum The minimum amount of WMON9 to unwrap
    /// @param recipient The address receiving MON
    function unwrapWMON9(uint256 amountMinimum, address recipient) external nonReentrant {
        uint256 balanceWMON = IWMON(WMON).balanceOf(address(this));
        if (balanceWMON < amountMinimum) revert InsufficientAmount();

        if (balanceWMON > 0) {
            IWMON(WMON).withdraw(balanceWMON);
            safeTransferMON(recipient, balanceWMON);
        }
    }
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant {
        ...
        // Need to reduce moonAmountBelongToMC.
        if (token == address(MOON)) {
            unchecked {
                // In fact balance should always be greater than or equal to moonAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balanceToken >= moonAmountBelongToMC) {
                    balanceToken -= moonAmountBelongToMC;
                } else {
                    // This should never happend.
                    moonAmountBelongToMC = balanceToken;
                    balanceToken = 0;
                }
            }
        }
    ...
    /// @param _amount The amount of moon injected.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        // Transfer moon token from receiver.
        MOON.safeTransferFrom(receiver, address(this), _amount);
        // Update moonAmountBelongToMC
        unchecked {
            moonAmountBelongToMC += _amount;
        }
        ...
        uint256 moonPerSecond;
        uint256 moonAmount = _amount;
        if (latestPeriodEndTime > currentTime) {
            uint256 remainingMoon = ((latestPeriodEndTime - currentTime) * latestPeriodMoonPerSecond) / PRECISION;
            emit UpdateUpkeepPeriod(latestPeriodNumber, latestPeriodEndTime, currentTime, remainingMoon);
            moonAmount += remainingMoon;
        }
        moonPerSecond = (moonAmount * PRECISION) / duration;
        unchecked {
            latestPeriodNumber++;
            latestPeriodStartTime = currentTime + 1;
            latestPeriodEndTime = endTime;
            latestPeriodMoonPerSecond = moonPerSecond;
        }
        emit NewUpkeepPeriod(latestPeriodNumber, currentTime + 1, endTime, moonPerSecond, moonAmount);
    }

    /// @notice Update moon reward for all the liquidity mining pool.
    ...
    /// @notice Update moon reward for the liquidity mining pool.
    ...
    /**
     * @notice Transfer MON in a safe way
     * @param to: address to transfer MON to
     * @param value: MON amount to transfer (in wei)
     */
    ...
    /// @notice Safe Transfer MOON.
    /// @param _to The MOON receiver address.
    /// @param _amount Transfer MOON amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = MOON.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            // Update moonAmountBelongToMC
            unchecked {
                if (moonAmountBelongToMC >= _amount) {
                    moonAmountBelongToMC -= _amount;
                } else {
                    moonAmountBelongToMC = balance - _amount;
                }
            }
            MOON.safeTransfer(_to, _amount);
        }
    }

    receive() external payable {
        if (msg.sender != address(nonfungiblePositionManager) && msg.sender != WMON) revert();
    }
}
