...
import './interfaces/external/IWMON.sol';
import './base/PoolInitializer.sol';

/// @title MonSwap Migrator
contract V3Migrator is IV3Migrator, PeripheryImmutableState, PoolInitializer, Multicall, SelfPermit {
    ...
    ) PeripheryImmutableState(_deployer, _factory, _WMON) {
        nonfungiblePositionManager = _nonfungiblePositionManager;
    }

    receive() external payable {
        require(msg.sender == WMON, 'Not WMON');
    }
    ...
    function migrate(MigrateParams calldata params) external override {
        ...
        if (amount0V3 < amount0V2) {
            ...
            if (params.refundAsETH && params.token0 == WMON) {
                IWMON(WMON).withdraw(refund0);
        ...
        if (amount1V3 < amount1V2) {
            ...
            if (params.refundAsETH && params.token1 == WMON) {
                IWMON(WMON).withdraw(refund1);
