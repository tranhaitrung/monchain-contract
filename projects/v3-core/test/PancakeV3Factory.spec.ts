...
import { PancakeV3Factory } from '../typechain-types/contracts/MonFactory.sol'
import { PancakeV3PoolDeployer } from '../typechain-types/contracts/MonPoolDeployer.sol'
...
describe('MonFactory.sol', () => {
  ...
  const fixture = async () => {
    const deployerFactory = await ethers.getContractFactory('MonPoolDeployer.sol')
    const factoryFactory = await ethers.getContractFactory('MonFactory.sol')
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('MonPool.sol')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('MonPool.sol')
