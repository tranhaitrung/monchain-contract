...
import '../base/core/libraries/LowGasSafeMath.sol';
import '../base/core/interfaces/IMonPool.sol';
...
library SmartRouterHelper {
    ...
    /// @param deployer The MonSwap V3 deployer contract address
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) public pure returns (IMonPool) {
        return IMonPool(computeAddress(deployer, getPoolKey(tokenA, tokenB, fee)));
    }

    /// @notice Returns the address of a valid MonSwap V3 Pool
    /// @param deployer The contract address of the MonSwap V3 deployer
    ...
    function verifyCallback(
        ...
        address tokenB,
        uint24 fee
    ) public view returns (IMonPool pool) {
    ...
    function verifyCallback(address deployer, PoolKey memory poolKey)
        public
        view
        returns (IMonPool pool)
    {
        pool = IMonPool(computeAddress(deployer, poolKey));
